...
TC = TelegramClient('bot', 22689950, "a4ba9b376c94a005435efb1079a6863f").start(bot_token=TOKEN)
with TC:
	asyncio.get_event_loop().run_until_complete(get_users(TC, CHAT_ID))
...
async def execution_add_button(message: types.Message, state: FSMContext):
	if len(message.text.split(' ')) != 1:
		...
		Data.queue.append(message.text)
		j.clear_duty_queue()
		make_duty(Data.queue, Data.period_for_one_person, j)
